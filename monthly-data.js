...
(function () {
  ...
  function renderTable(rows){
    ...
    tbody.innerHTML = rows.map(r=>{
      ...
      return `
        <tr data-id="${r.id}">
          <td class="px-4 py-3 whitespace-nowrap">${esc(r.tanggal_label||r.date)}</td>
          <td class="px-4 py-3 whitespace-nowrap">${esc(r.teknisi||"-")}</td>
          <td class="px-4 py-3 whitespace-nowrap">${esc(r.lokasi_dari||"-")}</td>
          <td class="px-4 py-3 whitespace-nowrap">${esc(r.lokasi_ke||"-")}</td>
          <td class="px-4 py-3 whitespace-nowrap">${esc(r.jenis||"-")}</td>
          <td class="px-4 py-3 whitespace-nowrap">${esc(r.detail||"-")}</td>
          <td class="px-4 py-3 whitespace-nowrap">${esc(r.status||"-")}</td>
          <td class="px-4 py-3 whitespace-nowrap">${esc(fmtJam(r.jam_masuk))}</td>
          <td class="px-4 py-3 whitespace-nowrap">${esc(fmtJam(r.jam_berangkat))}</td>
          <td class="px-4 py-3 whitespace-nowrap">${esc(fmtJam(r.jam_tiba))}</td>
          <td class="px-4 py-3 whitespace-nowrap">${esc(fmtJam(r.jam_mulai))}</td>
          <td class="px-4 py-3 whitespace-nowrap">${esc(fmtJam(r.jam_selesai))}</td>
          <td class="px-4 py-3 whitespace-nowrap">${esc(durasiPenyelesaianStr)}</td>
          <td class="px-4 py-3 whitespace-nowrap text-right">${r.jarak_km||0}</td>
          <td class="px-4 py-3 whitespace-nowrap">${esc(waktuTempuhStr)}</td>
          <td class="px-4 py-3 whitespace-nowrap">${esc(r.keterangan||"")}</td>
          <td class="aksi">
            <div class="flex items-center justify-center gap-1">
              <button class="btn-edit p-2 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-600 transition-colors" data-id="${r.id}" title="Edit" aria-label="Edit">
                <i data-lucide="file-pen-line" class="w-4 h-4"></i>
              </button>
              <button class="btn-del btn-monthly-del p-2 rounded-lg text-rose-500 hover:bg-rose-100 hover:text-rose-600 transition-colors" data-id="${r.id}" title="Hapus" aria-label="Hapus">
    ...
    tbody.querySelectorAll(".btn-del").forEach(btn=>{
      btn.addEventListener("click", async ()=>{
        ...
        if(!id) return;
        const confirmed = await window.showConfirm('Konfirmasi Hapus', 'Anda yakin ingin menghapus entri ini?', 'Ya, Hapus', 'danger');
        if (!confirmed) return;
  ...
  async function exportXLSX(){
    ...
    try{
      const confirmed = await window.showConfirm('Konfirmasi Export', 'Anda akan membuat file XLSX untuk bulan ini. Lanjutkan?', 'Ya, Export', 'info');
      if (!confirmed) return;
  ...
  async function resetMonth(){
    ...
    const confirmed = await window.showConfirm('Konfirmasi Reset', `Anda yakin ingin menghapus ${count} entri untuk bulan ${month}?`, 'Ya, Hapus Bulan Ini', 'danger');
    if (!confirmed) return;
